...
function code_manifest()
    files=[joinpath(studyroot,"src",f) for f in ("InformativeObservation.jl","run_study.jl","calibrate.jl","test_likelihood.jl")]
...
function run_one(job,mode,destination,manifest)
    ...
    result=try
        ...
        Dict("valid"=>false,"status"=>"exception","error"=>sprint(showerror,err),
          ...
          "nodes"=>C["estimation"]["gh_nodes"],"aware"=>false,"observed_rate"=>sum(data.observed)/length(data.observed),"elapsed_seconds"=>NaN)
    end
